...
const addProduct = asyncErrorHandler(async (req, res, next) => {
  ...
  try {
    ...
    const { name, description, price, cuttedPrice, category, stock, warranty } =
      sanitizedBody;
    ...
    const parsedHighlights = sanitizedBody.highlights
      ? JSON.parse(sanitizedBody.highlights)
      : [];
...
const updateProduct = asyncErrorHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.specifications) {
      updates.specifications = JSON.parse(req.body.specifications).map(
        (spec) => ({
          title: spec.title,
          description: spec.description,
        })
...
const getProductDetails = asyncErrorHandler(async (req, res, next) => {
  try {
    ...
    if (!product) {
      res
        .status(404)
        .json({ success: false, message: "unable to get product detail" });
    }
